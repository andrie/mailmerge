...
Write the text of your email as a R markdown document. You can add the subject line in the yaml header.  Use `{}` braces inside the email to refer to the data inside your data frame.  Expressions inside these braces will be encoded by the `glue::glue()` function (See https://glue.tidyverse.org/).
...
Then you can use `mail_merge()` to embed the content of your data frame into the email message.  Right now, the only supported email backend is `gmailr` (see https://gmailr.r-lib.org/).
